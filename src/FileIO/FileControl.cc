...
size_t FileControl::SaveEvt(std::ofstream &evtinfile, Event &evt) const{
    ...
    evtinfile.write((char*) pos,8*sizeof(float));
    delete [] pos;
    return 1;
}
...
int FileControl::ReadEvt(std::ifstream& evtinfile, Event &evt)const{
    ...
    evtinfile.read((char*) buffer,sizeof(float)*8);
    //evtinfile >> pos[0] >> pos[1] >> pos[2] >> pos[3] >> pos[4] >> pos[5] >> time_tof >>weight;

    Point3D pt1,pt2;
    pt1.setPoint(buffer[0],buffer[1],buffer[2]);
    pt2.setPoint(buffer[3],buffer[4],buffer[5]);
    evt.getRay().setRay(pt1,pt2);
    evt.settimeTOF(buffer[6]);
    evt.setweight(buffer[7]);
    return 1;
}
...
int FileControl::ReadEvt(std::ifstream &evtinfile, Point3D& pt1, Point3D& pt2, bool TOFflag, double &weight, double &time )const {
    if (!evtinfile.good()) {
        std::cerr << "ReadEvt(): Error while open file!" << std::endl;
        return -1;
    }
...
//        evt.setweight(buffer[7]);
        ...
        evtinfile.read((char*) buffer,sizeof(float)*7);
        pt1.setPoint(buffer[0],buffer[1],buffer[2]);
        pt2.setPoint(buffer[3],buffer[4],buffer[5]);
        weight=buffer[6];
//        evt.setweight(buffer[6]);
    }
    return 1;
}
