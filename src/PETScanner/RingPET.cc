...
void RingPET::TransEvents(const std::string& evtfilename, const std::string& transevtfilename){
    ...
    size_t nChords = fc.ReadEvtHdr(evtfilename);
    //read the event file.
    std::ifstream evtinfile(evtfilename,std::ios_base::in|std::ios_base::binary);
    ...
    for (int i = 0; i< nChords; i++){
        ...
        fc.ReadEvt(evtinfile, pt1, pt2,this->getTOFinfo().isopen(), weight, time);
        //fc.ReadEvt(evtinfile, pt1, pt2,this->getTOFinfo().isopen(), time1, time2);
        //std::cout<<pt1.getpx()<<" "<<pt1.getpy()<<" "<<pt1.getpz()<<std::endl;
